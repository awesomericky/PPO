...
class PPO_agent:
    def __init__(self, env, args):
        ...
        self.state_dim = env.obeservation_space.shape[0]
        try:
        ...
        self.gae_truncated_steps = args['gae_truncated_steps']
        ...
        with tf.variable_scope(self.name):
            ...
            entropy_bonus = -tf.reduce_maen(tf.exp(self.log_prob)*self.log_prob)
            if self.loss_type == 10:
                ...
                self.objective = tf.reduce_mean(prob_ratio*self.gaes) + entropy_bonus
            elif self.loss_type == 21:
                term_1 = prob_ratio*self.gaes
                term_2 = tf.clip_by_value(prob_ratio, clip_value_min=1-self.clipping_limit, clip_value_max=1+self.clipping_limit)*self.gaes
                self.objective = tf.reduce_mean(tf.reduce_min(tf.concat([term_1, term_2], axis=0), axis=0)) + entropy_bonus
            elif (self.loss_type == 31) or (self.loss_type == 41):
                self.objective = tf.reduce_mean(prob_ratio*self.gaes) - self.kl_coeff*self.kl + entropy_bonus
            else:
    ...
    def train(self, trajs):
        states = trajs[0]  # (num_steps, state_dim)
        actions = trajs[1]  # (num_steps, action_dim)
        v_targets = trajs[2]  # (num_steps,)
        gaes = trajs[3]  # (num_steps,)
        ...
        while start < total_trajs.shape[0] - self.batch_size:
            ...
            self.sess.run([self.mean, self.std, self.log_prob], feed_dict={
                self.states: batch_states,
